...
def sample_corridor_pillars(
    ...
    while len(pts) < num_pillars and tries < max_tries:
        tries += 1
        t = np.random.uniform(t_min, t_max)
        lateral = np.random.uniform(-corridor_half_width, corridor_half_width)

        p = start_xy + t * (goal_xy - start_xy) + lateral * normal
        ...
        # Inter-pillar distance check
        valid = True
        for q in pts:
            if np.linalg.norm(p - q) < min_pillar_dist:
                valid = False
                break
    ...
    if len(pts) < num_pillars:
        print(f"Warning: Only spawned {len(pts)}/{num_pillars} pillars in corridor after {max_tries} tries.")
...
def sample_random_field_metadata(
    ...
    if start is not None and goal is not None:
        ...
        pts = poisson_disk(num_pillars, region, min_dist, exclusion_zones=exclusion_zones)
